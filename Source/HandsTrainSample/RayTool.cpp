...
void ARayTool::Initialize_Implementation(UOculusHandComponent *HandComponent)
{
	Hand = HandComponent;
...
void ARayTool::Tick(float DeltaTime)
{
	...
	if (!IsValid(Hand) || !bIsInitialized ||
		!UOculusInputFunctionLibrary::IsPointerPoseValid(Hand->SkeletonType))
	{
		return;
	}
	...
	FTransform PointerPoseTransform =
		UOculusInputFunctionLibrary::GetPointerPose(Hand->SkeletonType);

...
void InteractableToolsInputRouter::UpdateTools(UOculusHandComponent* LeftHand,
	UOculusHandComponent* RightHand, const TSet<AInteractableTool*>& LeftHandNearTools,
	...
	const TSet<AInteractableTool*>& HandFarTools)
{
	...
	bool HandIsReliable = UOculusInputFunctionLibrary::GetTrackingConfidence(
		Hand->SkeletonType, 0) == ETrackingConfidence::High &&
		UOculusInputFunctionLibrary::IsHandTrackingEnabled();

	bool EncounteredNearObjectsHand = UpdateToolsAndGetEncounteredObjects(
		HandNearTools, HandIsReliable);

	bool PointerPoseIsValid = UOculusInputFunctionLibrary::IsPointerPoseValid(
		Hand->SkeletonType);
	/**

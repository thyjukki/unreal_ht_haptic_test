...
void AFingerTipPokeTool::Tick(float DeltaTime)
{
	...
	auto HandType = IsRightHandedTool ?
		EOculusHandType::HandRight : EOculusHandType::HandLeft;
	float CurrentHandScale = UOculusInputFunctionLibrary::GetHandScale(HandType);
...
void AFingerTipPokeTool::Initialize_Implementation(UOculusHandComponent *HandComponent)
{
	memset(VelocityFrames, 0, NumVelocityFrames * sizeof(FVector));

	BoneToTestCollisions = EBone::Pinky_3;
	switch (FingerToFollow)
	{
		case EHandFinger::Thumb:
			BoneToTestCollisions = EBone::Thumb_3;
			break;
		case EHandFinger::Index:
			BoneToTestCollisions = EBone::Index_3;
			break;
		case EHandFinger::Middle:
			BoneToTestCollisions = EBone::Middle_3;
			break;
		case EHandFinger::Ring:
			BoneToTestCollisions = EBone::Ring_3;
			break;
		default:
			BoneToTestCollisions = EBone::Pinky_3;
			break;
	...
	TArray<FOculusCapsuleCollider>& HandCapsules =
...
{
	float CurrentHandScale = UOculusInputFunctionLibrary::GetHandScale(
		IsRightHandedTool ? EOculusHandType::HandRight : EOculusHandType::HandLeft);

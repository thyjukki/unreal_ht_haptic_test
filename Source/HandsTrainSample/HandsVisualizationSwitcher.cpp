...
void AHandsVisualizationSwitcher::Tick(float DeltaTime)
{
	...
	bool bLeftHandConfidenceHigh = UOculusInputFunctionLibrary::IsHandTrackingEnabled() &&
		UOculusInputFunctionLibrary::GetTrackingConfidence(
			LeftHand->SkeletonType, 0) == ETrackingConfidence::High;
	UpdateHandBoneVisuals(LeftHand, LeftHandBoneInstancedMeshes,
		...
		UOculusInputFunctionLibrary::GetHandScale(LeftHand->SkeletonType));

	bool bRightHandConfidenceHigh = UOculusInputFunctionLibrary::IsHandTrackingEnabled() &&
		UOculusInputFunctionLibrary::GetTrackingConfidence(
			RightHand->SkeletonType, 0) == ETrackingConfidence::High;
	UpdateHandBoneVisuals(RightHand, RightHandBoneInstancedMeshes,
		...
		UOculusInputFunctionLibrary::GetHandScale(RightHand->SkeletonType));
	...
	float ProperAlpha = (CurrentVisualMode == EHandsVisualMode::Both) ?
		HandAlphaWhenBonesVisible : 1.0f;
	float LeftAlpha;
	float RightAlpha;
	if (CurrentVisualMode == EHandsVisualMode::Both &&
...
void AHandsVisualizationSwitcher::InitVisualsPerHand(UOculusHandComponent *Hand)
{
	FTransform StandardBoneTransform(FQuat::Identity, FVector::ZeroVector,
		FVector(1.0f, 0.01f, 0.01f));
	bool IsLeftHand = Hand->SkeletonType == EOculusHandType::HandLeft;
...
void AHandsVisualizationSwitcher::SwitchHandsVisualization()
{
	...
	CurrentVisualMode = (EHandsVisualMode)(
		(((int)CurrentVisualMode + 1) % ((int)EHandsVisualMode::Both + 1)));
	EnforceCurrentVisualMode();
}
